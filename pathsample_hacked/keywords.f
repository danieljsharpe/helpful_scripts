...
!
      ...
      INTEGER ITEM, NITEMS, LOC, LINE, NCR, NERROR, IR, NDUMMY2, LAST, FUNIT
      LOGICAL CAT
      ...
      DOUBLE PRECISION, ALLOCATABLE :: DUMMYLIST(:)
...
! sn402: Determine how frequenctly the pairlist will be regenerated by
! CONNECTREGION
      MAXCANDIDATES=1000

! sn402: keywords for extensions to the frustration metric
      SHANNON_EXCLUDET = .FALSE.
      SHANNON_NEXCLUDE = 0
      REFMINID = 0
...
C
      ELSE IF (WORD.EQ.'ADDPATH') THEN
         ADDPATH=.TRUE.
         IF (NADDPATH.GE.1000) THEN
            PRINT '(A)','keywords> Maximum number of ADDPATH lines is 1000 - skipping'
         ELSE
            NADDPATH=NADDPATH+1
            CALL READA(ADDPATHNAME(NADDPATH))
         ENDIF
C
...
C
C  Just run an optimised distance calculation for one minimum and some or all of the ts.
C
      ELSE IF (WORD.EQ.'DISTANCETS') THEN
         DISTANCETST=.TRUE.
         CALL READI(DISTANCETO)
         CALL READI(DISTANCETO1)
         CALL READI(DISTANCETO2)
...
C
      ELSE IF (WORD.EQ.'FREEZE') THEN
         ...
         IF (NITEMS.GT.1) THEN   ! Frozen atoms specified on keyword line
            DO J1=1,NITEMS-1
               NFREEZE=NFREEZE+1
               CALL READI(NDUMMY)
               FROZEN(NDUMMY)=.TRUE.
            ENDDO
         ELSE                    ! Frozen atoms specified in a file called 'frozen'
...
C
      ELSE IF (WORD.EQ.'KMCCOMMIT') THEN
         write(*,*) "Setting KMCCOMMITT to TRUE"
         NOPOINTS=.TRUE.
         KMCCOMMITT=.TRUE.
         IF (NITEMS.GT.1) CALL READF(NKMCCYCLES)
...
C
C calculate k shortest "distinct" paths (djs244)
C
      ELSE IF (WORD.EQ.'KDISTINCTPATHS') THEN
         KDISTINCTPATHST = .TRUE.
         CALL READI(KPATHS)
         CALL READI(RLEDGE_DEFN)
         IF (.NOT. (RLEDGE_DEFN==0 .OR. RlEDGE_DEFN==1 .OR. RLEDGE_DEFN==2)) THEN
             PRINT *, 'Fatal error: RLEDGE_DEFN must be 0,1 or 2'
             STOP
         ENDIF
         IF (NITEMS .GT. 3) THEN
            CALL READA(LOYNO)
            IF (LOYNO=="T") KDPDUMPEDGEST = .TRUE.
         ENDIF
         IF (NITEMS .GT. 4) THEN
            CALL READA(LOYNO)
            ! write cumulative min.data.fastest.x files, instead of individual
            IF (LOYNO=="T") KDPMDFCUMT = .TRUE.
         ENDIF
C
C dump input file for the InfoMap algorithm (djs244)
C
      ELSE IF (WORD .EQ. 'DUMPINFOMAP') THEN
          DUMPINFOMAPT = .TRUE.
          IF (NITEMS .GT. 1) THEN
            CALL READA(LOYNO)
            IF (LOYNO=="T") INFOMAPT = .TRUE.
          ENDIF
C
...
C
      ...
      ELSE IF (WORD.EQ.'MAXCANDIDATES') THEN
         ! Override the default maximum number of connection pairs
         ! computed in one call to CONNECTD. The default is 1000.
         CALL READI(MAXCANDIDATES)
...
!
     ...
     &                                MLPIN,MLPSTART,MLPHIDDEN,MLPOUT
         ...
         LUNIT=GETUNIT()
         OPEN(LUNIT,FILE='DLhiddendata',STATUS='OLD')
         READ (LUNIT,*) DLHIDDEN
         ALLOCATE(n_hidden(DLHIDDEN))
         READ (LUNIT,*) n_hidden(1:DLHIDDEN)
         WRITE(*,'(A,I10)') 'keywords> deep learning hidden layers=',DLHIDDEN
         WRITE(*,'(A)') 'keywords> deep learning hidden layer sizes:'
         WRITE(*,'(100I6)') n_hidden(1:DLHIDDEN)
         CLOSE(LUNIT)
         !ALLOCATE(DLxs(MLPDATA,MLPIN),DLys(MLPDATA,4))
         !DLxs = MLPDAT
         !DLys(:,1) = MLPOUTCOME
         !LUNIT=GETUNIT()
         !OPEN(LUNIT,FILE='./d02-data/train.csv',STATUS='OLD')
         !READ(LUNIT,*)
         !DO J1=1,21263
         !  READ(LUNIT,*) DLdata(J1,:)
         !ENDDO
         !CLOSE(LUNIT)
         !DLxs = DLdata(:,:81)
         !DLys = DLdata(:,82:)
         ...
         !ALLOCATE(n_hidden(DLHIDDEN))
         !n_hidden(1) = 4
         ALLOCATE(layers_width(0:SIZE(n_hidden)+1))
         layers_width(0) = MLPIN
         layers_width(SIZE(n_hidden)+1) = MLPOUT
         layers_width(1:SIZE(n_hidden)) = n_hidden(:)

         DLvars=SUM(layers_width(1:))+SUM(layers_width(:)*CSHIFT(layers_width(:),1))
     &                          -layers_width(0)*layers_width(SIZE(layers_width)-1)
         WRITE(*,'(A,I10)') 'keywords> number of DL variables=',DLvars
...
C
C  NEWREAXFF potential used, need to make sure we get the right files to
C  NEWREAXFF
C
...
C
      ...
      ELSE IF (WORD.EQ.'OVERWRITE_INERTIA') THEN
          OVERWRITE_INERTIA = .TRUE.   ! Used only with MERGEDB
...
!
      ELSE IF (WORD.EQ.'SHANNON') THEN
         ...
         IF (NITEMS.GT.5) CALL READF(MIN_EDIFF)
!
! Calculate a landscape Shannon entropy from harmonic equilibrium probabilities
...
!
! Specify a list of minima to be excluded from the frustration metric analysis - typically minima close to the global minimum in some sense.
!

      ELSE IF (WORD.EQ.'SHANNON_EXCLUDE') THEN
         SHANNON_EXCLUDET = .TRUE.
         OPEN(UNIT=1,FILE='shannon.exclude',STATUS='OLD')
         READ(1,*) SHANNON_NEXCLUDE
         ALLOCATE(SHANNON_EXCLUDELIST(SHANNON_NEXCLUDE), DUMMYLIST(SHANNON_NEXCLUDE))
         DO J1=1,SHANNON_NEXCLUDE
            READ(1,*) SHANNON_EXCLUDELIST(J1)
            DUMMYLIST(J1)=J1   ! I don't think this should be necessary
         ENDDO
         CLOSE(1)
         WRITE(*,*) "keywords> Read in list of minima to exclude from the frustation analysis. Sorting now"
 !        write(*,*) SHANNON_EXCLUDELIST(1:10)

         ! Sort the indices into ascending order. This is important when the list is very long!
         CALL SORT4(SHANNON_NEXCLUDE, SHANNON_NEXCLUDE, DUMMYLIST, SHANNON_EXCLUDELIST)

         WRITE(*,*) "keywords> Finished sorting"

!         write(*,*) SHANNON_EXCLUDELIST(1:10)
!         DO J1=2,SHANNON_NEXCLUDE
!            IF (SHANNON_EXCLUDELIST(J1).LE.SHANNON_EXCLUDELIST(J1-1)) THEN
!               write(*,*) "Error: Elements ", J1-1, "and ", J1
!               write(*,*) "are ", SHANNON_EXCLUDELIST(J1-1), "and ", SHANNON_EXCLUDELIST(J1)
!            ENDIF
!         ENDDO
         DEALLOCATE(DUMMYLIST)

      ELSE IF (WORD.EQ.'SHANNON_REFERENCE') THEN
         CALL READI(REFMINID)
...
C
      ELSE IF (WORD.EQ.'UNTRAPMETRIC') THEN
         ...
         ENDIF
C
C Launch OPTIM jobs using exec, to replace the child PATHSAMPLE instance, rather
C than launching OPTIM as a subprocess of the child PATHSAMPLE. SLURM only.
C
      ELSE IF (WORD.EQ.'USEEXEC') THEN
         IF (NITEMS .EQ. 0) THEN
            USEEXEC = .TRUE.
         END IF
         IF (NITEMS .GE. 1) THEN
            CALL READA(UNSTRING)
            IF (TRIM(ADJUSTL(UNSTRING)).EQ.'TRUE') USEEXEC=.TRUE.
            IF (TRIM(ADJUSTL(UNSTRING)).EQ.'FALSE') USEEXEC=.FALSE.
         END IF
C
...
! kr366: Changed directories for wilkes2 cluster
      ELSE IF (WORD.EQ.'WILKES') THEN
         WILKEST=.TRUE.
