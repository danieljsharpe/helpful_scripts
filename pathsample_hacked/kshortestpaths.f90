...
! Joanne M. Carr
!
! this version has been modified to print stochastic dynamic quantities for the paths
...
TYPE edge
   ...
   DOUBLE PRECISION :: k           ! djs244> (log) transition rate of the edge
   DOUBLE PRECISION :: k_rev       ! djs244> (log) transition rate of the reverse edge
   DOUBLE PRECISION :: ds          ! djs244> entropy flow along an edge
...
TYPE path
   ...
   DOUBLE PRECISION :: l           ! accumulated weight of path. djs244> (=path aciton)
   DOUBLE PRECISION :: t           ! djs244> average path time
   DOUBLE PRECISION :: s           ! djs244> path entropy flow (entropy production is negative of entropy flow plus boundary term)
...
! djs244> variables for other stochastic dynamic quantities besides path action
INTEGER ACTIVITY
DOUBLE PRECISION ENTROPY_FLOW, AVG_PATH_TIME
...
! Now convert the minim and ts arrays into the pointer-based derived data types.
      ...
      ! djs244>
      PRINT *, "kshortestpaths> setting up the edge weights, transition rates and other quantities"
      ! djs244> initialise array contianing the (log) escape rates for each node
      DO k1 = 1, NMIN
          ESC_RATES = -HUGE(0.D0)
      ENDDO
...
!         IF (MINUS(K1)==LJ2) THEN ! djs244> use this IF statement to include edges to initial node (LJ1)
         IF (.FALSE.) THEN
            ...
            ! djs244> set forward and reverse transition rates, and entropy flow
            tsedge(k1)%k = KPLUS(k1)
            tsedge(k1)%k_rev = KMINUS(k1)
            tsedge(k1)%ds = -(tsedge(k1)%k-tsedge(k1)%k_rev)
...
!         ELSEIF (PLUS(K1)==LJ2) THEN ! djs244> use this IF statement to include edges to initial node (LJ1)
         ELSEIF (.FALSE.) THEN
            ...
            ! djs244> set forward and reverse transition rates and entropy flow
            tsedge(nts+k1)%k = KMINUS(k1)
            tsedge(nts+k1)%k_rev = KPLUS(k1)
            tsedge(nts+k1)%ds = -(tsedge(nts+k1)%k-tsedge(nts+k1)%k_rev)
      ...
      ! djs244> some test printing for first shortest path (from my_dijkstra subroutine)
      PRINT *, "kshortestpaths> weight of the first shortest path:", shortest_paths(LJ2,1)%l
...
!         PRINT *,'kshortestpaths> shortest path: ',pp%k,' for start min ',LJ1,' and end min ',LJ2 ! djs244> comment out
         ...
         ! djs244> set other stochastic dynamic properties of path (besides action)
         ACTIVITY = pp%d
         ENTROPY_FLOW = pp%s
         AVG_PATH_TIME = pp%t
...
!         PRINT *, "kshortestpaths> I am going to write the path actions"
         ...
         ELSE
             OPEN(9,FILE="path_costs.dat",STATUS="new",ACTION="write")
         ENDIF
...
1995 CONTINUE ! djs244> this loop ensures that printing structures is skipped
...
! Initialization
    DO i = 1, nmin
       shortest_paths(i,1)%l = HUGE(0.0D0) ! djs244> NB this is the "path action"
       shortest_paths(i,1)%d = 0  ! djs244> dynamical activity
    ...
    DO n = 1, nmin                         ! Iterate nmin times
       ...
       DO j = 1, nmin
          ...
          IF (shortest_paths(j,1)%l < minweight) THEN
             ...
             ! djs244>
             IF (DEBUG) THEN
                 PRINT *, "kshortestpaths> debug> entropy flow of shortest path to node", j, "is:", shortest_paths(j,1)%s
             ENDIF
             minadj = j
          ENDIF
       ENDDO
...
! djs244> modified this subroutine to also pass changes in entropy flow (sweight) and average time (tweight)
...
! djs244> nullify stochastic dynamic quantities
...
USE GRAPH
USE COMMONS, ONLY : DEBUG ! djs244> added extra debug printing statements
IMPLICIT NONE

INTEGER :: v, k, start, u, k1, w
DOUBLE PRECISION :: weight

! djs244> additional arguments that get passed to the add_candidate subroutine that relate to stochastic dynamics
DOUBLE PRECISION :: sweight, tweight
...
IF (ASSOCIATED(shortest_paths(u,k1 + 1)%node)) THEN
   ! djs244> modified subroutine to take additional arguments sweight and tweight
   CALL add_candidate(v, u, k1 + 1, weight, sweight, tweight) ! (node, predecessor, which path the predecessor is from, weight)
ENDIF
