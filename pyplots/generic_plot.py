...
class PlotMulti(object):
    ...
    def plot(self,xlims,ylims,slabels,nxticks,nyticks):
        ...
        plt.ylabel("$\log_{10}(\mathrm{Total\ path\ prob.})$",fontsize=42)
        ax = plt.gca()
        ...
        ytick_labels = ["$"+"{:.0f}".format(ytick_val)+"$" for ytick_val in ytick_vals]
        ax.set_xticklabels(xtick_labels)
        ax.set_yticklabels(ytick_labels)
        # annotate: put labels on right hand side of figure adjacent to final data points, for each series
        for k in range(self.nseries):
            ax.annotate("$"+"{:.2f}".format(slabels[k])+"$",(1.01,self.data[k,-1]), \
                        xycoords=("axes fraction","data"),color=colors[k],fontsize=20)
        # save and show plot
        plt.tight_layout()
        plt.savefig("myfig.pdf",fmt="pdf",bbox_inches="tight")
        plt.show()
...
if __name__=="__main__":
    ...
    nent = 100000 # number of entries in each data file
    col = 4 # index (from 1) of column from which to read
    logvals = True
    ### PLOT PARAMS ###
    slabels = [0.1,0.5,1.0,1.5,1.75,2.0] # series labels for plot
    xlims = [0,100000]
    ylims = [-9.,0.]
    nxticks = 5
    nyticks = 9
    ...
    plotobj.data = np.exp(-1.*plotobj.data)
