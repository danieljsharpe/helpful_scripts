...
#                print("  ",x+i+1,x+j+1)
                ...
                if not (T[i,j]==0. and Tx==0.):
                    for Rmtx in Rmtxs: # loop over different reward matrices
                        ...
                        print("      indirect reward:\t",Rmtx[i,0]+Rmtx[0,j]+(((1./(1.-T[0,0]))-1.)*Rmtx[0,0]))
...
#        print("\nT:\n",T,"\nRs:\n",Rmtxs[1])
    ...
    for k, Rmtx in enumerate(Rmtxs): # recall only two states remain, initial state (idx 0) and absorbing state (idx 1)
        rewards[k] = (((1./(1.-T[0,0]))-1.)*Rmtx[0,0]) + Rmtx[0,1]
    return rewards
...
if __name__=="__main__":
    ...
    # transition probability matrix. The final state is the absorbing state and the script calculates the MFPT from a chosen initial state
    ...
    b = 4 # index of chosen initial state (from 1)
    tau = 0.05 # lag time (or mean waiting time for linearised transition matrix of CTMC)
    iterative=False # compute mean first passage rewards using iterative formulation of GT algo (T/F) (if F, use block formulation)
